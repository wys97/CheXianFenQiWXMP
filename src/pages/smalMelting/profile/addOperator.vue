			...
			<form  class="logo_form" @submit.prevent="submit" style="width: 100%;">
				<div class="input_groups">
					<div class="input_group" style="position: relative;">
						<i class="input_group_icon c_main mui-icon mui-icon-person"></i>
						<input class="input_group_input" v-model="user.name" @input="inputFunc" type="text"  placeholder="请输入操作员真实姓名" />
					</div>
					<div class="input_group" style="position: relative;">
						<i class="input_group_icon c_main iconfont icon-mobilefill"></i>
						<input class="input_group_input" v-model="user.mobile" @input="inputFunc" type="text" maxlength="11" onkeyup="value=value.replace(/[^\d]/g,'')"  placeholder="请输入操作员手机号码" />
					</div>
					<div class="input_group" style="position: relative;">
					...
					<div class="c_bdred" style="margin-top: .2rem;">成功添加后，该账号将可以在平台申请融资！</div>
					<div style="position: relative;height: 1.1rem;"></div>
				</div>
				
				<div style="width: 100%;text-align: center;">
					<input type="submit" class="gray_btn" v-bind:class="{black_login_btn: isSuccess }" value="保存" />
				</div>
			</form>  
	...
	export default {
		...
		methods: {
			fnGetCode: function() {
				...
				if(mflag) {
					that.$axios({//获取短信验证码
						url: '/supplier/login/send_sms',
						data: {
							mobile: that.user.mobile,
							smsType:'SUPPLIER_ADD_OPERATOR'
						}
					})
			...
			inputFunc: function() {
				//判断输入内容是否都为空
				var that = this;
				if(that.user.name!= null&that.user.mobile!= null&that.user.smsCode.length == 6) {
					this.isSuccess = true;
				} else {
			...
			passInputFunc:function(){
				//判断 账号密码输入内容是否都为空
				if(this.passUser.userName != null && this.passUser.password != null&&this.passUser.imageCode!=null) {
			...
			submit: function() {
				...
				if(mname==null){
					mui.toast(`
						<div  class="toast_content" >
					  	<i class="mui-icon iconfont icon-roundclose"></i>
					  	<div class="toast_text">姓名不能为空</div>
					</div>`);
					return false;
				}else if(!mflag) {
					mui.toast(`
					...
					this.$axios({
						url:'/supplier/profile/add_operator',
						data: {
							name: this.user.name,
